...
当遇到网络访问问题时（连接超时、429 错误、无法访问等），可能需要启用本地网络代理：

**常见表现**：
- 静态抓取返回 429 Too Many Requests
- curl/WebFetch 连接超时
- 特定站点无法访问

**解决方案**：
1. **启用本地代理**：`proxy-on` 或手动设置 `http_proxy`/`https_proxy` 环境变量
2. **CDP 浏览器模式**：使用用户日常 Chrome，天然携带代理设置

详细代理设置说明见 `references/cdp-api.md` 中的"代理设置"章节。

**常见需要代理的站点**：YouTube、Google、GitHub、Twitter 等国际站点。

**重要**：代理问题不是搜索能解决的。遇到网络访问失败时，优先检查是否需要启用代理，而非反复尝试其他搜索方式。
...
在执行 `/close` 前，**必须先完成'任务完成检查清单'**，确认是否需要沉淀站点经验。完成后关闭自己创建的 tab，保留用户原有的 tab 不受影响。
...
### 经验沉淀检查
- 本次操作是否涉及新站点？
- 是否发现了该站点的特殊模式（URL结构、选择器、反爬行为）？
- 是否遇到了需要记录的陷阱？

**如果以上任一项为"是"，必须写入站点经验文件**

### 写入格式
在 `references/site-patterns/{domain}.md` 中写入：
- 平台特征（架构、反爬、登录需求）
- 有效模式（URL模式、选择器、操作策略）
- 已知陷阱（什么会失败及原因）
- 标注更新日期
...
### 更新现有经验
如果站点经验文件已存在，但本次发现了新模式或新陷阱，追加到对应章节。
...
CDP 操作成功完成后，**必须执行经验沉淀检查**（见'任务完成检查清单'）。不要等用户提醒，主动判断并记录。经验是 skill 能力成长的核心，每次操作都是积累经验的机会。

**必须记录的情况**：
1. 首次操作某个站点 → 记录平台特征
2. 发现特殊 URL 结构或参数 → 记录有效模式
3. 遇到反爬/登录墙/动态加载 → 记录平台特征和已知陷阱
4. 找到了可靠的选择器 → 记录有效模式
5. 踩坑后找到解决方案 → 记录已知陷阱
